...
class EncodeNet(nn.Module):
    def __init__(self):
        ...
        self.conv_channels_up = nn.Conv2d(64, 6144, 1, groups=64)
        convList = []
        for i in range(3):
            convList.append(baseNet.SampleNet(downSample=True, in_channels=6144, out_channels=6144, groups=64))
...
class DecodeNet(nn.Module):
    def __init__(self):
        ...
        self.tconv_channels_down = nn.ConvTranspose2d(6144, 64, 1, groups=64)
        convList = []
        for i in range(3):
            convList.append(baseNet.SampleNet(downSample=False, in_channels=6144, out_channels=6144, groups=64))
...
DCTKernel = torch.from_numpy(dctTf.createDCTKernel(8)).float().cuda()
...
trainFrequencyData = torch.zeros(size=[batchSize * 4, 64, 16, 16]).cuda()
# 创建保存64种频率的矩阵
# (batchSize*4) * 1 * 128 * 128 -> (batchSize*4) * 64 * 16 * 16
...
for i in range(999999999):
    ...
    if (i == 0):
        ...
        testFrequencyData = torch.zeros(size=[testImgSum * 4, 64, 16, 16]).cuda()
        ...
        for ii in range(testGrayData.shape[0]):
            ...
            testFrequencyData[ii] = dctTf.splitDCTTensor(testGrayData[ii][0], 8)
    ...
    for ii in range(trainGrayData.shape[0]):
        ...
        trainFrequencyData[ii] = dctTf.splitDCTTensor(trainGrayData[ii][0], 8)
        # (batchSize*4) * 1 * 128 * 128 ->(batchSize*4) * 64 * 16 * 16
    ...
    if (i == 0):
        ...
        if (minTrainLoss > trainLoss and trainLoss<1000):  # 保存最小loss对应的模型
            ...
            for ii in range(decData.shape[0]):
                rebuildTestData[ii][0] = dctTf.mergeDCTTensor(decData[ii], 8)
    ...
    # print(sys.argv)
    print('训练到第%d次, 本次训练损失 = %.3f' % (i, trainLoss))
    print('最小训练损失 = %.3f, 最小测试损失 = %.3f, 上次保存模型时对应的训练次数为%d' % (minTrainLoss, minTestLoss, lastSavedI))
