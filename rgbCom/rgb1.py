...
class EncodeNet(nn.Module):
    def __init__(self):
        ...
        self.conv_down = baseNet.SampleNet(downSample=True, in_channels=64, out_channels=3)


    def forward(self, x):
        y = F.leaky_relu(self.conv_channels_up(x)) # c3 ->c64
        return F.interpolate(x, scale_factor=0.5, mode='bilinear') + F.leaky_relu(self.conv_down(y + self.convList(y)))

class DecodeNet(nn.Module):
    def __init__(self):
        super(DecodeNet, self).__init__()
        self.conv_up = baseNet.SampleNet(downSample=False, in_channels=3, out_channels=64)
    ...
    def forward(self, x):
        y = F.leaky_relu(self.conv_up(x))
        return F.interpolate(x, scale_factor=2, mode='bilinear') + F.leaky_relu(self.conv_channels_down(y + self.convList(y)))
...
for i in range(999999999):
    ...
    currentMSEL = F.mse_loss(trainData, decData)

    currentMS_SSIM = pytorch_msssim.ms_ssim(trainData, decData, win_size=7, data_range=255, size_average=True)
    ...
    if (i == 0):
        ...
        if (minLoss > loss):  # 保存最小loss对应的模型
            ...
            encData = encNet(testData / 255)
            qEncData = torch.zeros_like(encData)
            for ii in range(encData.shape[0]):
                qEncData[ii] = softToHardQuantize.sthQuantize(encData[ii], C, sigma)

            decData = decNet(qEncData) * 255
            testLoss = - pytorch_msssim.ms_ssim(testData, decData, win_size=7, data_range=255,
