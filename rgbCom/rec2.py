...
class RecNet(nn.Module):
    ...
    def forward(self, x):
        return F.leaky_relu_(self.conv_channels_down(self.convList(F.leaky_relu_(self.conv_channels_up((x - 127.5)/127.5)))))
...
for i in range(999999999):
    ...
    if (i == 0):
        ...
        if (minLoss > loss and loss < -0.9 ):  # 保存最小loss对应的模型
            ...
            testLoss = - pytorch_msssim.ms_ssim(testData, decData, data_range=1,
