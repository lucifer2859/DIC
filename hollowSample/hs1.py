...
import sys
sys.path.append('../')
import hollowSample.hollowSampleFunction as hollowSampleFunction
import os
...
class EncodeNet(nn.Module):
    def __init__(self):
        ...
        self.conv_channels_down = nn.Conv2d(256, 16, 1)


        self.convList0 = baseNet.ResNet(transpose=False, channels=256, kernel_size=3, padding=1)
        self.convList1 = baseNet.ResNet(transpose=False, channels=256, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.convList0(x)
        x = hollowSampleFunction.hollowDownSampleTensor(x)
        x = self.convList1(x)
...
for i in range(999999999):
    ...
    if (i == 0):
        ...
        if (minLoss > loss and loss < -0.8):  # 保存最小loss对应的模型
            # 在测试一组数据进行检测
