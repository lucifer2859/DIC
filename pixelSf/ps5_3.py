...
class EncodeNet(nn.Module):
    def __init__(self, channels):
        ...
        self.conv_channels_up = nn.Conv2d(4, channels, 1)
        convList = []
        for i in range(4):
        ...
        convList.append(nn.Conv2d(channels, 64, 1))
        convList.append(nn.LeakyReLU())
...
class DecodeNet(nn.Module):
    def __init__(self, channels):
        ...
        self.conv_channels_down = nn.Conv2d(channels, 4, 1)
        convList = []
        convList.append(nn.Conv2d(64, channels, 1))
        convList.append(nn.LeakyReLU())
        for i in range(4):
            convList.append(nn.BatchNorm2d(channels))
            convList.append(nn.Conv2d(channels, channels*4, 3, padding=1))
            convList.append(nn.LeakyReLU())
...
for i in range(999999999):
    ...
    if (i == 0):
        ...
        testGrayData = rgbCompress.RGB444DetachToRGGBTensor(testData, rgbKernelList, True, catDim=1)
        print('read all testData')

    optimizer.zero_grad()
    trainGrayData = rgbCompress.RGB444DetachToRGGBTensor(trainData, rgbKernelList, True, catDim=1)
