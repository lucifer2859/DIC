...
class SampleResNet(nn.Module): # 2层残差网络
    ...
    def forward(self, x):
        if(self.downSample==True):
            return self.convList(x)/16 + F.interpolate(x, scale_factor=0.5, mode='bilinear')
        else:
            return self.convList(x)/16 + F.interpolate(x, scale_factor=2, mode='bilinear')
...
class EncodeNet(nn.Module):
    def __init__(self, channels):
        ...
        convList = []
        for i in range(4):
            convList.append(SampleResNet(channels, True))
...
class DecodeNet(nn.Module):
    def __init__(self, channels):
        ...
        convList.append(nn.LeakyReLU())
        for i in range(4):
            convList.append(SampleResNet(channels, False))
