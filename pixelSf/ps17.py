...
class PixelAndConvSample(nn.Module):
    def __init__(self, downSample, channels):
        # 并行的卷积和像素重排
        super(PixelAndConvSample, self).__init__()
        self.downSample = downSample
        self.k = nn.Parameter(torch.zeros(1))
        if(self.downSample==True):
            self.convDown = nn.Conv2d(channels, channels, 2, 2)
            self.pixConv = nn.Conv2d(channels*4, channels, 3, padding=1)
        else:
            self.convUp = nn.ConvTranspose2d(channels, channels, 2, 2)
            self.pixConv = nn.Conv2d(channels, channels*4, 3, padding=1)


    def forward(self, input):
        if(self.downSample==True):
            return self.k * F.leaky_relu_(self.convDown(input)) + (1 - self.k) * F.leaky_relu_(self.pixConv(pixelShuffleFunction.pixel_unshuffle(input, 2)))
        else:
            return self.k * F.leaky_relu_(self.convUp(input)) + (1 - self.k) * F.leaky_relu_(F.pixel_shuffle(self.pixConv(input), 2))
...
class EncodeNet(nn.Module):
    def __init__(self, channels):
        ...
        convList = []
        for i in range(4):
            convList.append(PixelAndConvSample(True, channels))
...
class DecodeNet(nn.Module):
    def __init__(self, channels):
        ...
        for i in range(4):
            convList.append(nn.BatchNorm2d(channels))
            convList.append(PixelAndConvSample(False, channels))
...
if(sys.argv[2]=='0'): # 设置是重新开始 还是继续训练
    encNet = EncodeNet(256).cuda().train()
    decNet = DecodeNet(256).cuda().train()
